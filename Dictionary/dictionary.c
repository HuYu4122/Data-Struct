...
void aiv_set_insert(struct aiv_set *set, const char *key, const void *value)
{
...
void print_key_list(struct aiv_set * set)
{
    for(size_t size = 0; size < HASHMAP_SIZE; size++)
    {
        for(size_t i = 0; i < HASHMAP_SIZE_LIST; i++)
        {
            ...
            {
                printf("Key : %s - Value : %s\n", set->hashmap[size][i].key, set->hashmap[size][i].value);
            }
        }
    }
}
...
int main(int argc, char **argv)
{
    ...
    // Adding key
    printf("Adding key\n");
    aiv_set_insert(&myset, "Hello",     "Alfa");
    aiv_set_insert(&myset, "Hello2",    "56");
    aiv_set_insert(&myset, "Test",      "Beta");
    aiv_set_insert(&myset, "Foobar",    "32");
    aiv_set_insert(&myset, "Foobar",    "Gamma");
    aiv_set_insert(&myset, "XYZ",       "Delta");
    aiv_set_insert(&myset, "AAAAAA",    "12");
    aiv_set_insert(&myset, "AAAAAa",    "Epsilon");
